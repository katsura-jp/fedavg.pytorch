...
class FedBase:
    ...
    def create_mnist_datasets(self, num_clients=100, shard_size=300, datadir="./data/mnist", iid=False):
        ...
        print(f"divide data into {len(shard_start_index)} shards of size {shard_size}")
        ...
        for client_id in range(num_clients):
            ...
            img = np.concatenate([
                train_img[shard_start_index[_index+i]:shard_start_index[_index+i]+shard_size]
                for i in range(num_shards)
            ], axis=0)

            label = np.concatenate([
                train_label[shard_start_index[_index+i]:shard_start_index[_index+i]+shard_size]
                for i in range(num_shards)
            ], axis=0)
